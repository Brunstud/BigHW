...
public:
	//判断是否闭合
	int isClosed() { return (length > 0 ? closed : -1); };
	//作用：用离散点初始拟合一个曲线簇
...
/****************************************************************以下开始绘画*******************************************************************************************/
//随机出现的云朵
static void rand_clouds(const int width, const int height)
{
	srand((unsigned int)(time(0)));
	const int CLOUDS_COLOR = RGB_shade(rand() % 150 / 100.0, RGB(134, 186, 217), RGB(255, 255, 255));
	int num = rand() % 6 + 6;
	for (int i = 0; i < num; i++) {
		adaptor S(rand()%width, rand()%height, (rand() % 100 / 100.0 + 1), 0);
		int cld = rand() % 6 + 6;
		for (int j = 0; j < cld; j++)
			S.ad_ellipse(rand()%20, rand()%20, rand()%40 + 20, rand()%40 + 20, rand()%80 - 40, true, 2, CLOUDS_COLOR);
	}
}

//背景填充
static void background(const int width, const int height, const int cloud = 0)
{
	srand((unsigned int)(time(0)));
	const int BACKGROUND = RGB_shade(rand() % 100 / 100.0, RGB(151, 209, 250), RGB(202, 255, 255));
	hdc_rectangle(0, 0, width, height, 0, true, 2, BACKGROUND);
	if (cloud)
		rand_clouds(width, height);
}
...
//画面部背景
static void faceBase(const int base_x, const int base_y, const double size, const double rotation_angles)
{
	adaptor S(base_x, base_y, size, rotation_angles);
	//肤色填充
	...
	//右眼腮红
	S.ad_ellipse(60, -22, 24, 12, -10, true, 2, RGB(254, 234, 220));//腮红1
	S.ad_ellipse(60, -22, 16, 8, -10, true, 2, RGB(254, 222, 213));//腮红2
	S.ad_ellipse(60, -22, 8, 4, -10, true, 2, RGB(254, 217, 208));//腮红3
	//脸边线
...
static void face(const int base_x, const int base_y, const double size, const double rotation_angles)
{
	adaptor S(base_x, base_y, size, rotation_angles);
	//肤色填充
	S.ad_fun(faceBase, 0, 0, 1, 0);
	//五官
	S.ad_fun(left_eye, -54, -30, 0.5, 0);
	S.ad_fun(right_eye, 50, -54, 0.54, -5);
...
//随机出现的闪烁四芒星
static void rand_stars(const int base_x, const int base_y, const double size, const double rotation_angles)
{
	srand((unsigned int)(time(0)));
	...
	int num = rand() % 6 + 4;
	for (int i = 0; i < num; i++) {
		adaptor S(base_x + rand() % (base_x * 2 / 3) - base_x * 2 / 3, base_y + rand() % (base_y * 2 / 3) - base_y * 2 / 3, size * (rand() % 100 / 100.0 + 0.5), rotation_angles);
...
static void words(const int base_x, const int base_y, const double size, const double rotation_angles)
{
	...
	cct_gotoxy(base_x / 8 + !!(base_x % 8) - 5, base_y / 16 + !!(base_y % 16) - 22);
	std::cout << "今天的渣哥";
	cct_gotoxy(base_x / 8 + !!(base_x % 8) - 8, base_y / 16 + !!(base_y % 16) - 21);
	std::cout << "不是我北黑的对手!";
	cct_setcolor();
}
...
{
	cct_setcolor(COLOR_HWHITE, COLOR_HPINK);
	cct_gotoxy(base_x / 8 + !!(base_x % 8) - 14, base_y / 16 + !!(base_y % 16) + 15);
	std::cout << "body();//这里空间太小，放不下";
	cct_setcolor();
}
...
{
	cct_gotoxy(0, HEIGHT / 16 - 3);
	std::cout << "这里有疑似作者留下的秘密记录：\n哥渣的给lib快画确的实，但出画来の际果实效不还如【#@%&】的";
}
...
void hdc_draw_cartoon_2252201(const int base_x, const int base_y)
{
	...
	S.ad_fun(hairBH, 0, 0, 1.2, 0);
	S.ad_fun(hairBase, 0, 0, 1.2, 0);
	S.ad_fun(hairEar, 0, 0, 1.2, 0);
	Sleep(WAIT);
	background(WIDTH, HEIGHT, 1);

	//测试曲线
	//S.ad_fun(testCurve, 0, 0, 1.6, 0);

	//演示头部
	S.ad_fun(head, 0, 100, 1, 0);
	S.ad_fun(rand_stars, 0, 0, 1, 0);
	S.ad_fun(words, 0, 0, 1, 0);
