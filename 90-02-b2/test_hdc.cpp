...
int main(int argc, char** argv)
{
	...
	if (test_choise[T_MATHCURVE] == '1') {
		...
		struct {
			...
			{2252201, "胡世成", hdc_draw_Mathematical_curve_2252201},
			{0,NULL,NULL}
		};
	...
	if (test_choise[T_CUSTOM] == '1') {
		...
		struct {
			...
			{2252201, "胡世成", hdc_draw_cartoon_2252201, 400, 400},
			{0,NULL,NULL}
		};
		/* 显示显示助教哥哥/姐姐们画的卡通图像
		   注：调用函数前，先要添加对应的cpp并在.h中加函数声明
		*/
		for (i = 0; hdc_draw_cartoon[i].sno > 0; i++) {
			...
			to_be_continued("卡通绘图-", hdc_draw_cartoon[i].name, "-完成", STL_X, STL_Y);
		}
...
//	hdc_set_delay(delay_ms);
	...
	//test_cartoon(520, 500);
	hdc_draw_cartoon_2252201(400, 400);
