...
{
	int dot_count = 0;//记录点数
	const char* pcheck = ipstr, * pnum = ipstr;
	for (pcheck = pnum = ipstr; *pcheck; pcheck++) {
		if ('.' == *pcheck) {//遇到'.'就对前面的数字进行检验
			int num = atoi(pnum);
			if (*pnum < '0' || *pnum > '9'|| num > 255)
				return false;
			pnum = pcheck + 1;
			dot_count++;
		}
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	/* 打印参数的值 */
	cout << "参数检查通过" << endl;
	cout << " --t参数：" << args[ARGS_T].existed() << endl;
